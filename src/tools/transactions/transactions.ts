...
export function addStateUpdateField(transaction: TTransaction & WithApiMixin & IWithApplicationStatus): TTransaction & WithApiMixin & IWithApplicationStatus {
    if (transaction.type === TRANSACTION_TYPE.INVOKE_SCRIPT && transaction.stateChanges.invokes.length) {
    ...
    } if (transaction.type === TRANSACTION_TYPE.INVOKE_EXPRESSION && transaction.stateChanges.invokes && transaction.stateChanges.invokes.length) {
        const payments = (transaction as any).payment ? (transaction as any).payment.map((p: TPayment) => ({
            assetId: p.assetId,
            amount: p.amount
        })) : []
